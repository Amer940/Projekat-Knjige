...
const IzmijeniKnjigu = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try{
      ...
      const apiOdgovor = await axios.put("http://localhost:8800/api/knjige/edit/"+id.id, {knjiga, knjigaUrl}, {withCredentials: true});
  ...
  useEffect(() => {
    const getKnjiga = async () => {
      try{
        const ans = await axios.get("http://localhost:8800/api/knjige/dohvati/"+id.id, {withCredentials: true});
        setKnjiga({
          name: ans.data.name,
          quantity: ans.data.quantity
        })
      }catch(err){
        console.log(err);
      }
    ...
    <>
      <div className='container flex items-center justify-center h-screen'>
        <div className='bg-white rounded-[18px] p-12 flex flex-col min-w-[570px]'>
          <span className='text-2xl mb-10'>Promijeni informacije knjige</span>
          <form onSubmit={handleSubmit}>
            <div className='relative'>
              <span className='left-[8px] top-[-5px] absolute px-0.5 leading-none bg-white text-[12px]'>Ime Knjige</span>
              <input placeholder={`${knjiga.name}`} className='border border-[#081225] rounded-lg  pt-2 pl-2 p-1 w-full' type="text" name="name" onChange={handleChange}/>
            </div>
            <div className='relative'>
              <span className='left-[8px] top-[10px] absolute px-0.5 leading-none bg-white text-[12px]'>Stanje</span>
              <input placeholder={`${knjiga.quantity}`} className='mt-4 border border-[#081225] rounded-lg pt-2 pl-2 p-1 w-full' type="number" name="quantity" onChange={handleChange}/>
            </div>
            <div className='relative'>
              <span className='left-[8px] top-[10px] absolute px-0.5 leading-none bg-white text-[12px]'>Dodaj sliku</span>
              <input className='mt-4 border border-[#081225] rounded-lg pt-2 pl-2 p-1 w-full' type="file" onChange={e=>setImg(e.target.files[0])}/>
            </div>
